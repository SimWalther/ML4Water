...
class Gaussian_Weighted_Sum(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.s = self.add_weight(
            shape=(self.units,),
            initializer=keras.initializers.RandomNormal(mean=1.0, stddev=0.5),
            trainable=True,
        )
    ...
    def call(self, inputs, training=None):
        if training:
            ...
            rel = tf.keras.ops.concatenate(
                [keras.ops.ones(1), rel]
            )  # relative importance of first Gaussian is always 1
            loc = (
                self.n * tf.keras.ops.sigmoid(self.w) + 0.5
            )  # parameter normalization (sigmoid * range)
            weights = tf.keras.ops.exp(
                -((self.l - loc) ** 2) / (self.s**2)
            )  # weight computation (Gaussian)
            weights = tf.keras.ops.multiply(weights, rel)  # weighting weights
            weights = tf.keras.ops.sum(
                weights, axis=-1, keepdims=True
            )  # weight merging (multimodal)
            weights = weights / tf.keras.ops.sum(
...
def create_model(
    ...
    dropout2,
    lr,
    loss_to_use="mean_absolute_error",
):
    inputs, processed_inputs = [], []
    ...
    for group_name, X_group in X_group_dict.items():
        ...
        if group_type == "gaussian":
            processed = Gaussian_Weighted_Sum(units=5, name=f"gaussian_{group_name}")(
                input_layer
            )
        elif group_type == "softmax":
    ...
    model = keras.models.Model(inputs=inputs, outputs=output)
    model.compile(loss=loss_to_use, optimizer=Adam(learning_rate=lr), metrics=["mae"])
