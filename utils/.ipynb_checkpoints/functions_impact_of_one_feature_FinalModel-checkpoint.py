...
def prepare_grouped_data(
    ...
    and prepare grouped feature arrays for the custom model.
    """
    # --- Train/test split ---
    df_train = df[~(df['station'] == test_station)].copy()
    df_test = df[df['station'] == test_station].copy()
...
def get_hot_days(df, threshold_Ta):
    ...
    """
    mask = df["air_max_temp"] > threshold_Ta
    hot_dates = df.loc[mask, "date"].unique().strftime("%Y-%m-%d").tolist()
...
def upstream_feature_sensitivity_with_training(
    df,
    upstream_slavi_df,
    altitude_diff,
    model,
    ...
    """
    Varies one upstream feature in upstream_slavi_df between vmin and vmax,
    recreates Training instances for each value, and evaluates the trained model.

    Returns arrays of feature values, mean/std predictions, and original value.
    """
    ...
    # # --- original value(s) ---
    # original_values = upstream_slavi_df.loc[
    #     (upstream_slavi_df["station"] == station_name) &
    #     (upstream_slavi_df["date"].isin(target_days)),
    #     feature_of_interest
    # ].values
    ...
    for value in feature_values:
        print(value)
        # --- 1) modify the upstream_slavi_df for this value ---
        modified_upstream = upstream_slavi_df.copy()
        modified_upstream.loc[(modified_upstream["station"] == station_name),feature_of_interest] = value

        # --- 2) create new Training instance ---
        training_mod = Training(
            df=df,
            upstream_slavi_df=modified_upstream,
        ...
        # --- 3) build model inputs for these days/station ---
        # Get all stations
        all_stations = df["station"].unique()
        
        # Define training vs test
        train_stations = [s for s in all_stations if s != station_name]
        test_stations = [station_name]
        
        # --- build inputs ---
        inputs = training_mod.create_model_inputs(
            train_stations=train_stations,
            test_stations=test_stations
        )
        
        test_df = inputs["test_df"]
        test_dates_str = test_df["date"].dt.strftime("%Y-%m-%d").tolist()

        # Find indexes of rows corresponding to target_days
        target_days_str = [pd.to_datetime(d).strftime("%Y-%m-%d") for d in target_days]
        target_indexes = [i for i, d in enumerate(test_dates_str) if d in target_days_str]
        
        # Filter each element of test_inputs by these indexes
        test_inputs_filtered = [
            inp[target_indexes] if isinstance(inp, np.ndarray) else inp for inp in inputs["test_inputs"]
        ]
        ...
        preds = model.predict(test_inputs_filtered, verbose=0).flatten()
    ...
    if plot:
        ...
        # ax.axvline(x=original_values[0], color="k", ls="--", label="Original Value")
        ax.set_title(f"Station: {station_name}, Days: {len(target_days)}")
        ax.set_xlabel(feature_of_interest)
        ax.set_ylabel("Predicted Water Temperature (°C)")
        ax.legend()
        ax.grid(True)
    ...
    return feature_values, mean_predictions, std_predictions,0 #original_values
