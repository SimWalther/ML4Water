...
def compute_initial_weights(model, X_train_list, y_train_scaled, feature_groups, group_definitions):
    """
    ...
    for this_group in tqdm(feature_groups.keys(), desc="Extracting initial weights"):
        ...
        if layer_type == 'gaussian':
            ...
            initial_weights = this_layer.get_weights()[0].flatten() #this_layer.get_weights_normalized().flatten()
...
def plot_predictions_over_time(y_test, y_pred, test_days, station_name):
    ...
    ax.plot(test_days, y_pred, label='Predicted Water Temperature', linestyle='-', color='cyan')
